...
import {
  ...
  createDeliveryPackageType,
  updateDeliveryPackageType,
  deleteDeliveryPackageType,
...
type Tab = "analytics" | "packages" | "surge" | "cancellation";
...
export default function DeliveryManagement() {
  ...
  // Package Types
  const [packages, setPackages] = useState<DeliveryPackageType[]>([]);
  const [pkgModal, setPkgModal] = useState(false);
  const [editingPkg, setEditingPkg] = useState<DeliveryPackageType | null>(null);
  ...
  const loadAnalytics = async () => {
    ...
    try {
      const res = await getDeliveryAnalytics({ range });
  ...
  const loadPackages = async () => {
    try {
      const res = await getDeliveryPackageTypes();
      setPackages(res.data.data?.packageTypes || res.data.data || []);
    } catch {
  ...
  const loadSurges = async () => {
    try {
      const res = await getSurgeConfigs("delivery");
  ...
  const loadPolicies = async () => {
    try {
      const res = await getCancellationPolicies("delivery");
  ...
  const openPkgCreate = () => {
    setEditingPkg(null);
    setPkgForm({ name: "", description: "", maxWeight: 5, baseFare: 0, perKmRate: 0, perStopCharge: 0, minimumFare: 0, displayOrder: 0, isActive: true });
    setPkgModal(true);
  };

  const openPkgEdit = (p: DeliveryPackageType) => {
    setEditingPkg(p);
    setPkgForm({
      name: p.name,
  ...
  const savePkg = async () => {
    try {
      if (editingPkg) {
        await updateDeliveryPackageType(editingPkg._id, pkgForm);
        toast.success("Package type updated");
      } else {
        await createDeliveryPackageType(pkgForm);
  ...
  const removePkg = async (id: string) => {
    if (!confirm("Delete this package type?")) return;
    try {
      await deleteDeliveryPackageType(id);
  ...
  const saveSurge = async () => {
    try {
      const payload = { ...surgeForm, serviceType: "delivery" };
      if (editingSurge) {
  ...
  const savePolicy = async () => {
    try {
      const payload = { ...policyForm, serviceType: "delivery" };
      if (editingPolicy) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Truck className="h-7 w-7 text-orange-600" /> Delivery Management
          </h1>
          <p className="text-sm text-gray-500 mt-1">Manage delivery pricing, package types, surge rules, and cancellation policies</p>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
        {([
          ...
          { key: "packages", label: "Package Types", icon: Package },
      ...
      {tab === "analytics" && (
        <div className="space-y-6">
            ...
            {["today", "7d", "30d"].map((r) => (
              <button
                ...
                className={`px-3 py-1.5 text-sm rounded-lg ${
                  range === r ? "bg-orange-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                }`}
              >
          ...
          {loadingAnalytics ? (
            <div className="flex justify-center py-12"><div className="h-8 w-8 animate-spin rounded-full border-4 border-orange-600 border-t-transparent" /></div>
          ) : analytics ? (
            <>
              {/* KPI Cards */}
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {[
                  { label: "Total Orders", value: st?.totalOrders || 0, icon: Package, color: "orange" },
                  ...
                  { label: "Avg Fare", value: `₹${Math.round(st?.avgFare || 0)}`, icon: IndianRupee, color: "purple" },
                ].map((kpi) => (
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-xl border p-5">
                  <h3 className="text-sm font-semibold text-gray-700 mb-4">By Package Type</h3>
                  <ResponsiveContainer width="100%" height={280}>
                ...
                <div className="bg-white rounded-xl border p-5">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold">{st?.totalOrders || 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500">Completed</span>
                      <span className="font-semibold text-green-600">{st?.completed || 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500">Cancelled</span>
                      <span className="font-semibold text-red-600">{st?.cancelled || 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500">Revenue</span>
                      <span className="font-semibold text-emerald-600">₹{Math.round(st?.totalRevenue || 0)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500">Avg Fare</span>
                      <span className="font-semibold">₹{Math.round(st?.avgFare || 0)}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {tab === "packages" && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={openPkgCreate} className="flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-sm font-medium">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Max Weight</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Base Fare</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Per Km</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Per Stop</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Min Fare</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {packages.length === 0 ? (
                  <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-400">No package types configured</td></tr>
                ) : packages.map((p) => (
                  <tr key={p._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{p.maxWeight} kg</td>
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">₹{p.baseFare}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">₹{p.perKmRate}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">₹{p.perStopCharge}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">₹{p.minimumFare}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 text-xs rounded-full ${p.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                        {p.isActive ? "Active" : "Inactive"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => openPkgEdit(p)} className="p-1.5 text-gray-400 hover:text-blue-600"><Edit2 className="h-4 w-4" /></button>
                      <button onClick={() => removePkg(p._id)} className="p-1.5 text-gray-400 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {tab === "surge" && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={openSurgeCreate} className="flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-sm font-medium">
      ...
      {tab === "cancellation" && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={openPolicyCreate} className="flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-sm font-medium">
      ...
      {pkgModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold">{editingPkg ? "Edit" : "Add"} Package Type</h2>
              <button onClick={() => setPkgModal(false)} className="text-gray-400 hover:text-gray-600"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Name</label>
                <input className="w-full border rounded-lg px-3 py-2 text-sm" value={pkgForm.name} onChange={(e) => setPkgForm((f) => ({ ...f, name: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
                <input className="w-full border rounded-lg px-3 py-2 text-sm" value={pkgForm.description} onChange={(e) => setPkgForm((f) => ({ ...f, description: e.target.value }))} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Max Weight (kg)</label>
                  <input type="number" step="0.5" min="0" className="w-full border rounded-lg px-3 py-2 text-sm" value={pkgForm.maxWeight} onChange={(e) => setPkgForm((f) => ({ ...f, maxWeight: parseFloat(e.target.value) }))} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Base Fare (₹)</label>
                  <input type="number" min="0" className="w-full border rounded-lg px-3 py-2 text-sm" value={pkgForm.baseFare} onChange={(e) => setPkgForm((f) => ({ ...f, baseFare: Number(e.target.value) }))} />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Per Km (₹)</label>
                  <input type="number" min="0" className="w-full border rounded-lg px-3 py-2 text-sm" value={pkgForm.perKmRate} onChange={(e) => setPkgForm((f) => ({ ...f, perKmRate: Number(e.target.value) }))} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Per Stop (₹)</label>
                  <input type="number" min="0" className="w-full border rounded-lg px-3 py-2 text-sm" value={pkgForm.perStopCharge} onChange={(e) => setPkgForm((f) => ({ ...f, perStopCharge: Number(e.target.value) }))} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Min Fare (₹)</label>
                  <input type="number" min="0" className="w-full border rounded-lg px-3 py-2 text-sm" value={pkgForm.minimumFare} onChange={(e) => setPkgForm((f) => ({ ...f, minimumFare: Number(e.target.value) }))} />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Display Order</label>
                <input type="number" min="0" className="w-full border rounded-lg px-3 py-2 text-sm" value={pkgForm.displayOrder} onChange={(e) => setPkgForm((f) => ({ ...f, displayOrder: Number(e.target.value) }))} />
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button onClick={() => setPkgModal(false)} className="px-4 py-2 text-sm border rounded-lg hover:bg-gray-50">Cancel</button>
                <button onClick={savePkg} className="px-4 py-2 text-sm bg-orange-600 text-white rounded-lg hover:bg-orange-700">{editingPkg ? "Update" : "Create"}</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-4">
                  ...
                  {DAYS.map((d, i) => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-lg text-xs font-medium ${
                        surgeForm.conditions.daysOfWeek.includes(i) ? "bg-orange-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                      }`}
                    >
                      {d}
                    </button>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={saveSurge} className="px-4 py-2 text-sm bg-orange-600 text-white rounded-lg hover:bg-orange-700">{editingSurge ? "Update" : "Create"}</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={addRule} className="text-xs text-orange-600 hover:text-orange-800 flex items-center gap-1"><Plus className="h-3 w-3" /> Add Rule</button>
                </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={savePolicy} className="px-4 py-2 text-sm bg-orange-600 text-white rounded-lg hover:bg-orange-700">{editingPolicy ? "Update" : "Create"}</button>
              </div>
            </div>
          </div>
        </div>
